...
/// This function is to place order.
/// what we do here is to display the available products and using the .prompt()
///  method we can allow the user to select the product they want to order
/// 
pub fn place_order(){
    ...
    match order {
        Ok(amount) => {
            if let Some(a) = amount {
                ...
                println!("Your total is: {} satoshi", a);
            };
        },
        Err(e) => {
            eprintln!("{}", e);
